...
- 1 pool model not calculating systemAge. need to figure out why if this is of interest 
...
- Crop residue to soil (1)
  - assume cereal grains have negligible P returns to soil pools, but need a value so that pools have input and output
...
to_unavailable = 1 #small flux so that pools have input and output
plant_uptake = 13.95 #plant uptake from available pool
from_litter = 1 #small flux so that pools have input and output
...
FP_3_pool_scaled = matrix(c(
    -(veg_loss + from_litter), plant_uptake, 0, # From (1) Vegetation
    
    from_litter, -(plant_uptake + subsurface_flow_scaled + to_unavailable), to_available, # From (2) Plant Available
